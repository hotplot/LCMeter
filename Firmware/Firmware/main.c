...
ISR(TIMER0_COMPA_vect) {
	...
	if (T0SoftCount == 25) {
		...
		// Jitter is summed separately and ignores the frequency difference between readings 0 and 9 since those readings are not consecutive
		Jitter = 0;
...
static void updateDisplays() {
	...
	lcd_goto(0x40);
	char *freqStr = format("F", currentFreq, "Hz");
	lcd_puts(freqStr);
	free(freqStr);
}
